...
export function parseImportedResume(rawText, options = {}) {
  const legacy = autoImproveResume(localOptimizeResume(rawText, options.targetRole || ""), options.targetRole || "");
...
export function buildResumeHtmlPages(modelOrResume) {
  ...
  return (model.pages || [])
    .map((page, pageIndex) => {
      ...
      const content = page.map((item) => renderResumeHtmlBlock(item)).join("");
...
function renderResumeHtmlBlock(item) {
  ...
  if (item.type === "contact") return `<p class="resume-contact">${escapeHtml(item.text)}</p>`;
  if (item.type === "heading") return `<h2 class="resume-heading">${escapeHtml(item.text)}</h2>`;
  if (item.type === "entry-title") {
    const { left, right } = splitEntryTitleText(item.text);
    return `<div class="resume-entry-title"><span class="resume-entry-left">${escapeHtml(left)}</span>${right ? `<span class="resume-entry-right">${escapeHtml(right)}</span>` : ""}</div>`;
  }
...
function normalizeSection(section, order = 0) {
  ...
  return {
    ...
    title: String(safeSection.title || SECTION_LIBRARY[safeSection.type]?.title || "SECTION").trim(),
...
function buildDocumentBlocks(resume, typography, pageSpec) {
  ...
  const contact = [profile.phone, profile.email, profile.location, profile.linkedin, profile.github, profile.portfolio, profile.website].filter(Boolean).join(" | ");
  if (contact) blocks.push(block("contact", contact, typography.bodySize, 1.2));
...
function sectionToBlocks(section, typography, pageSpec) {
  ...
  (section.content?.entries || []).forEach((entry) => {
    ...
    if (titleLine.trim()) blocks.push(block("entry-title", titleLine, typography.bodySize, 0.35));
  ...
  (section.content?.blocks || []).forEach((item) => {
    ...
    if (item.type === "entry" && item.entry) {
      ...
      if (titleLine) blocks.push(block("entry-title", titleLine, typography.bodySize, 0.35));
      const metaLine = [item.entry.organization, item.entry.subtitle].filter(Boolean).join(" | ");
      if (metaLine) blocks.push(block("entry-meta", metaLine, typography.bodySize, 0.35));
...
function block(type, text, size, after) {
  return { type, text, size, after, lines: [] };
}
...
function buildDocumentXml(model, resume) {
  ...
  model.pages.forEach((page, pageIndex) => {
    page.forEach((item) => {
      if (item.type === "heading") body.push(docxParagraph(item.lines.join(" "), { style: "SectionHeading" }));
      ...
      else if (item.type === "entry-title") body.push(docxEntryTitleParagraph(item.text, model));
      else if (item.type === "entry-meta") body.push(docxParagraph(item.lines.join(" "), { style: "MetaLine" }));
...
<w:styles xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
  ...
  <w:style w:type="paragraph" w:styleId="SectionHeading">
    ...
    <w:rPr><w:b/><w:caps/><w:sz w:val="${headingHalfPoints}"/></w:rPr>
  </w:style>
...
function docxEntryTitleParagraph(text, model) {
  const [left, right] = String(text || "").split(" || ").map((item) => String(item || "").trim());
  const pageSpec = PAGE_SPECS[model.pageSize === "US Letter" ? "LETTER" : "A4"];
  ...
  return `<w:p>
    ...
    <w:r><w:rPr><w:b/></w:rPr><w:t xml:space="preserve">${escapeXml(left)}</w:t></w:r>
